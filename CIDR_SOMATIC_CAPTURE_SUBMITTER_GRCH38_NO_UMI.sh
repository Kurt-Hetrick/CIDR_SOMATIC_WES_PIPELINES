...
########################################################################
	...
	COMMON_SCRIPT_NO_UMI_DIR="${SUBMITTER_SCRIPT_PATH}/common_scripts_no_umi"
...
##################################################
# make an array for each PROJECT in sample sheet #
##################################################
...
##################################
	...
	MAKE_PROJ_DIR_TREE ()
	{
		...
		${CORE_PATH}/${SEQ_PROJECT}/REPORTS/{ALIGNMENT_SUMMARY,ANEUPLOIDY_CHECK,ANNOVAR,COUNT_COVARIATES,GATK_CALC_TUMOR_CONTAM,ERROR_SUMMARY,LAB_PREP_REPORTS,PICARD_DUPLICATES,QC_REPORTS,QC_REPORT_PREP,QUALITY_YIELD,RG_HEADER,TI_TV,TI_TV_MS,VERIFYBAMID,VERIFYBAMID_CHR} \
...
##################################################################################
	...
	##############################################################################
		...
		SELECT_VERIFYBAMID_VCF ()
		{
			...
			${HG19_SCRIPT_DIR}/A01-A01-SELECT_VERIFYBAMID_VCF.sh \
				${ALIGNMENT_CONTAINER} \
				${CORE_PATH} \
				${PROJECT} \
				${SM_TAG} \
...
####################################################################
	...
	RUN_BWA ()
	{
		...
		${COMMON_SCRIPT_NO_UMI_DIR}/A02-BWA.sh \
			${ALIGNMENT_CONTAINER} \
			${CORE_PATH} \
			${PROJECT} \
			${FCID} \
...
#########################################################################################
	...
	# What is being pulled out of the merged sample sheet
		...
		| awk 'BEGIN {FS="\t"} \
			...
			"'${COMMON_SCRIPT_NO_UMI_DIR}'""/B01-MARK_DUPLICATES.sh",\
			"'${ALIGNMENT_CONTAINER}'",\
			"'${CORE_PATH}'",\
			$1,\
			$2,\
...
###########################################
	...
	###############################################
		...
		DOC_CODING ()
		{
			...
			-hold_jid E02-BAM_TO_CRAM_${SGE_SM_TAG}_${PROJECT} \
	...
	#########################
		...
		GET_PILEUP_SUMMARY ()
		{
			...
			qsub \
				${STD_QUEUE_QSUB_ARGS} \
			-N F04-GET_PILEUP_SUMMARY_${SGE_SM_TAG}_${PROJECT} \
				-o ${CORE_PATH}/${PROJECT}/LOGS/${SM_TAG}/${SM_TAG}-COLLECT_HS_METRICS.log \
			-hold_jid A01-FIX_BED_FILES_${SGE_SM_TAG}_${PROJECT},E02-BAM_TO_CRAM_${SGE_SM_TAG}_${PROJECT} \
		...
		######################################
			...
			GATK_TUMOR_CONTAMINATION_ESTIMATION ()
			{
				...
				qsub \
					${STD_QUEUE_QSUB_ARGS} \
				-N F04-A01-GATK_CALC_TUMOR_CONTAM_${SGE_SM_TAG}_${PROJECT} \
					-o ${CORE_PATH}/${PROJECT}/LOGS/${SM_TAG}/${SM_TAG}-GATK_CALC_TUMOR_CONTAM.log \
				-hold_jid F04-GET_PILEUP_SUMMARY_${SGE_SM_TAG}_${PROJECT} \
...
printf "${SAMPLE_SHEET}\nhas finished submitting at\n`date`\nby `whoami`" \
	| mail -s "${PERSON_NAME} has submitted CIDR_SOMATIC_CAPTURE_SUBMITTER_GRCH38_NO_UMI.sh" \
		${SEND_TO}
