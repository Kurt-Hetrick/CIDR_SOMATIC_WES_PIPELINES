...
# Performs concordance between one vcf file and one final report. The vcf must be single sample.
	...
	CMD="${JAVA_1_8}/java -jar"
		...
		# [1] path_to_vcf_file
		CMD=${CMD}" ${CORE_PATH}/${PROJECT}/TEMP/${SAMPLE_SHEET_NAME}/${SM_TAG}/${SM_TAG}_QC_PASS_OnTarget_SNV_LIFTOVER_GRCH38_PRIMARY.vcf"
		# [2] path to the final report
		CMD=${CMD}" ${CORE_PATH}/${PROJECT}/TEMP/${SAMPLE_SHEET_NAME}/${SM_TAG}/"NoAlts"_${FINAL_REPORT_FILENAME}"
		# [3] path_to_bed_file
		CMD=${CMD}" ${CORE_PATH}/${PROJECT}/TEMP/${SAMPLE_SHEET_NAME}/${SM_TAG}/${SM_TAG}-${TARGET_BED_NAME}-LIFT_GRCH38_PRIMARY.bed"
