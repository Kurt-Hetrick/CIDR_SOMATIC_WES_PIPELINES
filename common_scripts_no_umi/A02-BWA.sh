...
# I got files from yale, that used the illumina naming conventions and actually went a step farther and broke files by tile (i think).
	...
	then
		...
		FASTQ_1=`(ls -tr $CORE_PATH/${PROJECT}/FASTQ/{${FIXED_PLATFORM_UNIT}_1.fastq*,${FIXED_PLATFORM_UNIT}_R1_000.fastq*,${SM_TAG}_R1_001.fastq*,${SM_TAG}_1.fastq*} 2> /dev/null | tail -n 1)`

		FASTQ_2=`(ls -tr $CORE_PATH/${PROJECT}/FASTQ/{${FIXED_PLATFORM_UNIT}_2.fastq*,${FIXED_PLATFORM_UNIT}_R2_000.fastq*,${SM_TAG}_R2_001.fastq*,${SM_TAG}_2.fastq*} 2> /dev/null | tail -n 1)`
	fi
