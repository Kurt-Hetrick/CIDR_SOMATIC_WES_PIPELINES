...
START_VCF_METRICS_BAIT=$(date '+%s') # capture time process starts for wall clock tracking purposes.
	...
	# construct command line
		...
		CMD=${CMD}" CollectVariantCallingMetrics"
			...
			CMD=${CMD}" --TARGET_INTERVALS ${CORE_PATH}/${TUMOR_PROJECT}/TEMP/${QC_REPORT_NAME}/${TUMOR_INDIVIDUAL}/${TUMOR_INDIVIDUAL}_${TUMOR_SM_TAG}_${NORMAL_SM_TAG}_${BAIT_BED}-picard.bed"
			CMD=${CMD}" --DBSNP ${DBSNP}"
