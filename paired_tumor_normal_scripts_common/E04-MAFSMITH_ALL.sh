...
START_MAFSMITH_ALL_MAF=$(date '+%s') # capture time process starts for wall clock tracking purposes.

	# construct command line

		CMD="singularity exec ${MAFSMITH_CONTAINER} mafsmith"
			CMD=${CMD}" vcf2maf"
				CMD=${CMD}" --input-vcf ${CORE_PATH}/${TUMOR_PROJECT}/VCF/MUTECT2/${TUMOR_INDIVIDUAL}_${NORMAL_SM_TAG}_${TUMOR_SM_TAG}_MUTECT2.FILTERED.vcf.gz"
		...
		CMD=${CMD}" --output-maf ${CORE_PATH}/${TUMOR_PROJECT}/REPORTS/MAFSMITH/${TUMOR_INDIVIDUAL}_${NORMAL_SM_TAG}_${TUMOR_SM_TAG}_MAFSMITH_ALL.maf"
...
END_MAFSMITH_ALL_MAF=$(date '+%s') # capture time process starts for wall clock tracking purposes.

# write out timing metrics to file

	echo ${TUMOR_INDIVIDUAL}_${TUMOR_PROJECT}_${TUMOR_SM_TAG}_${NORMAL_SM_TAG},D01,MAFSMITH_ALL_MAF,${HOSTNAME},${START_MAFSMITH_ALL_MAF},${END_MAFSMITH_ALL_MAF} \
